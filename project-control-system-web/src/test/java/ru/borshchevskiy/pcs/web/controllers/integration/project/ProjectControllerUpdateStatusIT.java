...
class ProjectControllerUpdateStatusIT extends IntegrationTestBase {
    ...
    public void updateStatus() throws Exception {
        ...
        ProjectStatusDto request = new ProjectStatusDto();
        request.setStatus(ProjectStatus.DEVELOPMENT);

        ProjectDto expectedDto = new ProjectDto();
        expectedDto.setId(1L);
        expectedDto.setCode("project1");
        expectedDto.setName("Project 1");
        expectedDto.setStatus(ProjectStatus.DEVELOPMENT);


        mockMvc.perform(post("/api/v1/projects/1/status")
                        .with(user(account))
    ...
    public void notFound() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/projects/" + Long.MIN_VALUE + "/status")
                        .with(user(account))
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    @Test
    public void changeToNull() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/projects/1/status")
                ...
                .andExpect(MockMvcResultMatchers.content().contentType("text/plain;charset=UTF-8"))
                .andExpect(MockMvcResultMatchers.content()
                        .string("Status can't be null."));
    }

    @Test
    public void changeNotByChain() throws Exception {

        ProjectStatusDto request = new ProjectStatusDto();
        request.setStatus(ProjectStatus.TESTING);
        ...
        mockMvc.perform(post("/api/v1/projects/1/status")
                ...
                .andExpect(MockMvcResultMatchers.content().contentType("text/plain;charset=UTF-8"))
                .andExpect(MockMvcResultMatchers.content()
                        .string("Current status " + dto1.getStatus() +
                                " can only be changed to " +
                                ProjectStatus.values()[dto1.getStatus().ordinal() + 1]));
    }

    // Тестируем на project 3
    @Test
    public void tryToChangeFinalStatus() throws Exception {

        ProjectStatusDto request = new ProjectStatusDto();
        request.setStatus(ProjectStatus.TESTING);

        mockMvc.perform(post("/api/v1/projects/3/status")
                        .with(user(account))
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().is4xxClientError())
                .andExpect(MockMvcResultMatchers.content().contentType("text/plain;charset=UTF-8"))
                .andExpect(MockMvcResultMatchers.content()
                        .string("Current status " + dto3.getStatus() +
